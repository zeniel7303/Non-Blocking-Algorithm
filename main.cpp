...
큐는 멀티쓰레드 프로그래밍에서 자주 쓰인다. 쓰레드끼리 데이터를 주고받을 때, 옆 쓰레드 데이터를 줄 떄까지
멍하니 기다리고 있다면 블로킹이며 CPU 낭비이다. 상대방 쓰레드가 받을 준비가 될 떄까지 기다리고 있다가 준비가 되고
던져주는 것도, 기다리는 상황 자체가 CPU 낭비가 된다. 그래서 큐를 일종의 버퍼로 생각해서 줄 게 있으면 받든 말든
버퍼에 넣어두고 다른 일을 한다.
...
/*
3. ABA 문제
Queue에는 문제가 있다. 쓰레드가 늘어날수록 성능이 좋아지지 않는다. 자료구조 자체가 병렬성과 관계가 없는 자료구조이므로
아무리 락프리로 한다해도 성능이 좋아지지 않는다. 락프리는 쓰레드끼리 계속 충돌하면서 순서가 뒤로 밀리기 떄문이다.
그럼 정해진 기간 안에 모두 다 끝나도록 Wait-Free로 구현해야하는가?
웨잇프리의 정해진 기간은 쓰레드가 많을 수록 정해진 기간이 늘어난다. 그때 웨잇프리 만능 알고리즘을 보면,
apply 호출이 정해진 시간에 끝난다. 여기서 정해진 시간은 쓰레드 갯수에 따라 정해진다. 그러므로 웨잇 프리로 한다고 해도
기아 상태는 없어지나 쓰레드가 늘어날수록 성능이 나아지지 않는 것은 똑같다. 그럼 쓰레드 갯수에 비례해 성능이 좋아지는 큐는 없는가?
많은 연구가 있음에도 제대로된 결과가 나온 것은 없다. 근본적으로 큐는 멀티쓰레드에서 돌린다고 성능이 빨라지고 그런 거랑 관계가 없다.

- ABA
1) 노드의 재사용 시 생기는 문제
	[1] new(), free()는 메모리를 재사용한다.
	[2] CAS 사용 시 다른 쓰레드에서 그 주소를 재사용했을 가능성이 있다.

지금까지 멀티쓰레드 프로그래밍에서 주의해야할 요소들이 있었다. (프로그램 이상 동작 방지)
1) 컴파일러 문제
	- 컴파일러가 최적화하면서 메모리 읽고 쓰는 것을 건너뛴다. 이건 volatile을 사용해 컴파일러가 이상하게 최적화 하는 것을 강제로 막았다.

2) 메모리 순서 문제
	- 메모리에 분명 썻는데 썼다는 것을 전달받지 못한다. 먼저 쓰고 읽으라고 했는데 CPU는 일단 읽고 나중에 쓰는 방식으로 엉망으로 순서를 바꾼다.
	  이건 atomic_thread_fence를 사용해 해결.
	참고 링크 : https://popcorntree.tistory.com/91, https://1-taek-gameprogramming.tistory.com/37

3) 중간값 문제
	- 캐시를 읽을 때 메모리버스가 한방에 읽으면 무슨 값이 들어있든 그 값을 정확히 있는다. 그래서 캐시 라인에 걸쳐있다면
	  반 읽고 나머지 반을 읽는 두 단계가 필요하다. 근데 그 사이에 다른 쓰레드가 끼어들 수 있어 업데이트될 때 중간에 읽을 수 있고
	  읽는 중간에 끼어들어 다른 쓰레드가 업데이트할 수도 있다. 이걸 막는 방법은 그냥 자료구조의 주소를 주의하는 수 밖에 없다.
	  일반적인 자료구조는 컴파일러가 알아서 잘한다. 예로 char로 구조체를 만들어도 위험하지 않다. 1바이트여도 다 4의 배수로 저장되므로
	  size가 4를 차지하고 1바이트만 데이터고 나머지는 빈 공간으로 둔다. 이것이 패딩(padding)이다.
	  포인터 가지고 더하고 빼고 데이터를 다룰 떄만 조심하면 된다. 포인터 연산을 하지 않으면 신경 쓰지 않아도 된다.

4) ABA 문제 (메모리 재사용 시 생기는 문제)
	- 지금까지 delete를 제대로 구현하지 않거나 주석으로 지워놨다. 오작동때문이였다.
	  알고리즘만 보면 오작동할 이유가 없어보이는데 왜 오작동하는가?
	  이야기를 처음으로 돌아가서 delete는 해줘야한다. 메모리를 계속해서 잡아먹기 떄문이다. 그래서 넣고 실행하면 문제없이 잘 돌아간다 싶을 수 있다.
	  1만 개를 미리 큐에 쌓아두고 시작하므로 endq와 deq가 만날 일이 적었기 때문이다. 그러나 그 수를 줄이면 deq에서 계속 에러가 날 것이다.
	  노드 재사용을 해야겠다. 안하면 당연히 메모리 릭이고, free list를 만들거나 관리를 해줘야하는데 큐 같은 경우엔
	  알고리즘 상에 문제가 없다. 그러나 특별하게 오작동하는 케이스가 있고 이 것을 ABA라고 한다. 
	  알고리즘 자체는 문제가 없으나 락프리 알고리즘 자체에 문제가 있는 것이다.

	  (리소스 파일 다운로드_2.png 참고)

	  큐를 하고 있다. 헤드가 있고 테일이 있다. 여러 쓰레드(A, B, C)가 있고 헤드와 테일이 CAS하며 동작하고 있다.
	  뭐가 문제인 것인가?
	  쓰레드 A가 deq 했다. 그러면? head를 뒤쪽으로 이동시켜야한다. 값을 읽고 head를 이동하고, delete 해줘야한다.
	  이 상황에서 쓰레드 A가 하는 일은 CAS를 해 성공하면 리턴하는 것이다. 실패하면 다시 해야지 하고 CAS를 실행한다.
	  쓰레드 A는 head가 a면 b로 바꿔라 라는 CAS를 실행 후 밑으로 내려간다. 그런데 CAS하겠다고 했는데 여기서 딜레이가 생긴다.
	  컨텍스트 스위칭이 일어나거나 해서 잠시 CAS 하기 전에 멈춘다. 그 동안 쓰레드 B와 C는 계속 동작한다.
	  그리고 B나 C가 그 사이에 deq로 a를 날리고 b를 날리고 노드를 계속 날린다. 다시 A가 시도했을 때 바뀌어있으면 실패할태니 상관없을거라 생각하면서.
	  그리고 쓰레드 B나 C가 deq했으니 enq하려할 떄 뭘 enq 할 것인가? a는 쓰레드 B나 C에서 delete해 메모리 풀에 들어가 있고,
	  enq해서 new 했는데 아까 delete한 메모리를 재활용하기위해 다시 enq할 것이다. 그러면 a가 붙게 된다.
	  그리고 A가 다시 일어난다. 그리고 컨택스트 스위칭 전에 하려던 CAS를 시도한다. head가 a를 가리키고 있다. 그리고 b로 바꾼다.
	  근데 b는 이상한 값인지 정상적인 값인지 모른다. 그럼 head가 엉뚱한 값을 가리키게 될 수 있다.
	  이미 delete된 값, 아까 a와 함께 delete되었던 b, head가 a를 가리켰다가 b를 가리켰다가 다시 a를 가리킨다. 그래서 aba 문제가 한다.
	  메모리 재사용을 하다보면 생기는 문제이다.
	  CAS(&head, a, b) 이렇게 했을때 a가 안전장치인데 다른 쓰레드가 new delete 하다가 안전장치가 풀린 것이다. 그래서 오작동하는 것이 aba 문제이다.
	  이건 CAS를 사용하면서 메모리를 재사용하기 떄문에 생기는 문제다. 또한 CAS를 사용하는 모든 락프리 알고리즘이 갖는 문제이기도 하다.
	  그렇다고 메모리 재사용을 하지 말아야하는 것은 아니다. 해결법이 여러개 있다.

- ABA 문제 해결법
1) 포인터를 포인터 + 스탬프로 확장해 포인터 값을 변경할 때마다 스탬프 값을 변경시킨다.
	- 설명
2) LL, SC 명령을 사용한다.(ARM, Alpha, PowerPC)
	- 설명
3) Reference Counter를 사용한다.
	- 설명
4) Java는 Reference Counter를 사용하는 Garbage Collection을 사용하므로 이러한 문제가 없다.
	- 설명
 */